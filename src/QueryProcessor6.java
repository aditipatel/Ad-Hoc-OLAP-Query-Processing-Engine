...
public class QueryProcessor6{

	class MFStructure{
		public String prod;
		public int month;
		public int avg_quant_1;
			...
			for(String havingCond : havingCondArr){
				if(havingCond.contains("[")){
					...
					if(leftExpArr[0].contains("MF.")){
						...
						leftleftVal = Integer.parseInt(leftExpArr[0]);
					if(leftExpArr[2].contains("MF.")){
						...
						leftrightVal = Integer.parseInt(leftExpArr[2]);
					String leftOp = leftExpArr[1];
	...
	public static void main(String args[])throws Exception{
		QueryProcessor6 q = new QueryProcessor6();
		List<Sales> salesList = q.getSalesData();
		List<MFStructure> mfList = q.populateMFStructure(salesList,false,"prod,month,avg_quant_1,count_quant_2","prod,month","{year}[==]{1997}");
		q.processPredicates(mfList,salesList,"{MF.prod.avg_quant_1}[=]{prod}:{MF.month.avg_quant_1}[=]{month},{MF.prod.count_quant_2}[=]{prod}:{MF.month.count_quant_2}[=]{month}:{MF.avg_quant_1.count_quant_2}[>]{quant}","{year}[==]{1997}","prod,month","avg_quant_1,count_quant_2");
		mfList = q.processHavingClause(mfList,"{MF.count_quant_2,>,0}");
